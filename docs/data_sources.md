...
窗口成交额 = sum(窗口内日成交额)
成交均值占比 = 窗口成交额 / 窗口交易日数量 / 窗口期初 ETF 规模 * 100
```
...
- 展开图展示 `5日滑动窗口成交均值占比`，每个点使用该 5 日窗口成交额日均值除以该滚动窗口期初 ETF 规模。

该指标用于观察二级市场日均交易热度和流动性活跃程度，不等同于一级市场 ETF 净申购资金流。API 中 `turnover_yi` 保留窗口累计成交额，`turnover_ratio` 为窗口成交额日均值占期初规模比例。若成交额字段缺失，则对应占比显示为 `--`，不按 0 处理。
