...
窗口起点价格优先使用同日 `fund_nav.unit_nav`；若净值缺失，则使用同日 `fund_daily.close` 估算。
...
当日期初 ETF 规模优先使用上一个 ETF 交易日份额乘以上一个 ETF 交易日单位净值，净值缺失时使用收盘价估算。遇到份额分拆、合并或折算时，按可比价格处理。
...
最新价格优先使用同日 `fund_nav.unit_nav`；若净值缺失，则使用最新 `fund_daily.close` 估算。该口径更接近机构使用的 ETF AUM 口径；当净值尚未发布时，页面会临时使用收盘价估算。
